          ...
          {activeTab === "rejected" && (
            <>
              ...
              <button
                ...
                className="flex-1 flex items-center justify-center gap-2 bg-green-500 text-white font-bold py-3 rounded-xl hover:bg-green-600"
              >
                <CheckCircle size={16} /> Move to Approved
              </button>
              <button
                onClick={() => {
                  onDelete(memory.id);
...
const AdminDashboard = () => {
  ...
  const fetchRejected = async () => {
    ...
    try {
      const res = await fetchWithNgrok(
        `${API_URL}/api/admin/memories/rejected`,
      );
      const data = await res.json();
      if (data.success) setMemories(data.memories);
    } catch (err) {
  ...
  const fetchStats = async () => {
    try {
      const [p, a, r] = await Promise.all([
        ...
        fetchWithNgrok(`${API_URL}/api/admin/memories/rejected`).then((r) =>
          r.json(),
        ),
      ]);
      setStats({
  ...
  useEffect(() => {
    const handleClick = (e) => {
      ...
      if (
        approvedDateRef.current &&
        !approvedDateRef.current.contains(e.target)
      )
        setApprovedDateFilterOpen(false);
    };
  ...
  const handlePermanentDelete = async (id) => {
    ...
    try {
      await fetchWithNgrok(
        `${API_URL}/api/admin/memories/${id}/permanent-delete`,
        { method: "DELETE" },
      );
      setMemories(memories.filter((m) => m.id !== id));
      fetchStats();
  ...
  const handleSetWinner = async (id) => {
    ...
    try {
      await fetchWithNgrok(`${API_URL}/api/admin/memories/${id}/set-winner`, {
        method: "PATCH",
      });
  ...
  const handleSetFunniest = async (id) => {
    if (!window.confirm("Set this memory as Most Funniest?")) return;
    try {
      await fetchWithNgrok(`${API_URL}/api/admin/memories/${id}/set-funniest`, {
        method: "PATCH",
      });
      alert("Funniest memory set successfully!");
      fetchApproved();
    } catch (err) {
      console.error(err);
    }
  ...
  const handleSetHeartwarming = async (id) => {
    if (!window.confirm("Set this memory as Most Heartwarming?")) return;
    try {
      await fetchWithNgrok(
        `${API_URL}/api/admin/memories/${id}/set-heartwarming`,
        { method: "PATCH" },
      );
      alert("Heartwarming memory set successfully!");
      fetchApproved();
    } catch (err) {
      console.error(err);
    }
  ...
  const getFilteredMemories = () => {
    ...
    if (activeTab === "approved" && approvedDateFilter !== "all") {
      ...
      if (approvedDateFilter === "today") {
        ...
        filtered = filtered.filter(
          (m) =>
            new Date(m.created_at) >= yesterday &&
    ...
    if (activeTab === "approved" && approvedFilter !== "all") {
      ...
      if (approvedTimePeriod === "today") {
        ...
        filtered = filtered.filter(
          (m) =>
            new Date(m.created_at) >= start && new Date(m.created_at) <= end,
        );
      }
      if (approvedFilter === "liked")
        filtered = filtered
          .sort((a, b) => (b.likes || 0) - (a.likes || 0))
          .slice(0, 10);
      else if (approvedFilter === "shared")
        filtered = filtered
  ...
  const handleBulkMoveToPending = async () => {
    if (!window.confirm(`Move ${selectedIds.length} memories to Pending?`))
      return;
    try {
      await Promise.all(
        selectedIds.map((id) =>
          fetchWithNgrok(`${API_URL}/api/admin/memories/${id}/pending`, {
            method: "PATCH",
          }),
        ),
      );
  ...
  const handleBulkMoveToApproved = async () => {
    if (!window.confirm(`Move ${selectedIds.length} memories to Approved?`))
      return;
    try {
      await Promise.all(
        selectedIds.map((id) =>
  ...
  const handleBulkPermanentDelete = async () => {
    if (!window.confirm(`Permanently delete ${selectedIds.length} memories?`))
      return;
    try {
      await Promise.all(
        selectedIds.map((id) =>
  ...
  const handleExportCSV = (winnersOnly = false) => {
    const dataToExport = winnersOnly
      ? memories.filter((m) => m.is_winner)
      : memories;
    const headers = [
      "Name",
      "Email",
          ...
          {["pending", "approved", "rejected"].map((tab) => (
            ...
            >
              ...
              {tab === "rejected" && stats.rejected > 0 && (
                <span
                  className={`text-xs px-2 py-0.5 rounded-full font-bold ${activeTab === "rejected" ? "bg-white text-purple-600" : "bg-red-500 text-white"}`}
                >
                  {stats.rejected}
                </span>
              )}
          ...
          {activeTab === "approved" && (
            <>
              ...
              <div ref={approvedDateRef} className="relative">
                <button
                  onClick={() =>
                    setApprovedDateFilterOpen(!approvedDateFilterOpen)
                  }
                  className="flex items-center gap-2 bg-white border border-gray-200 text-purple-600 font-semibold text-sm px-3 py-1.5 rounded-full hover:bg-purple-50"
                >
                  {datePeriodLabels[approvedDateFilter]}
                  <ChevronDown
                    size={13}
                    className={`transition-transform ${approvedDateFilterOpen ? "rotate-180" : ""}`}
                  />
                </button>
                  ...
                  <div className="absolute top-10 right-0 bg-white rounded-2xl shadow-xl border border-gray-100 z-50 min-w-40 py-2">
                    {[
                      ...
                      <button
                        key={opt.value}
                        onClick={() => {
                          setApprovedDateFilter(opt.value);
                          setApprovedDateFilterOpen(false);
                          setCurrentPage(1);
                        }}
          ...
          <div className="bg-green-50 border border-green-200 rounded-xl px-4 py-3 flex items-center justify-between flex-wrap gap-2">
            ...
            <div className="flex gap-2 flex-wrap">
              <button
                ...
                className="flex items-center gap-1 bg-yellow-500 text-white text-sm font-semibold px-4 py-1.5 rounded-lg hover:bg-yellow-600"
              >
        ...
        <div className="px-4 sm:px-8 mb-4">
          <div className="bg-red-50 border border-red-200 rounded-xl px-4 py-3 flex items-center justify-between flex-wrap gap-2">
            ...
            <div className="flex gap-2 flex-wrap">
              ...
              <button
                onClick={handleBulkMoveToApproved}
                className="flex items-center gap-1 bg-green-500 text-white text-sm font-semibold px-4 py-1.5 rounded-lg hover:bg-green-600"
              >
                <CheckCircle size={14} /> Move to Approved
              </button>
              <button
      ...
      <div className="px-4 sm:px-8 pb-8">
        {loading ? (
          ...
          (() => {
            ...
            const paginatedMemories = filteredMems.slice(
              ...
              <div className="bg-white rounded-2xl shadow-sm overflow-hidden border border-gray-100">
                ...
                <div className="hidden sm:grid grid-cols-12 gap-2 px-6 py-3 bg-gradient-to-r from-purple-50 to-pink-50 border-b border-gray-100 text-xs font-black text-purple-600 uppercase tracking-wide items-center">
                  ...
                  {approvedFilter === "liked" && (
                    <div className="col-span-1 flex items-center gap-1 text-red-500">
                      <Heart size={13} className="fill-red-500" /> Likes
                    </div>
                  )}
                  {approvedFilter === "shared" && (
                    <div className="col-span-1 flex items-center gap-1 text-purple-500">
                      <Share2 size={13} /> Shares
                    </div>
                  )}
                  <div className={hasExtraCol ? "col-span-2" : "col-span-2"}>
                    Title
                  </div>
                ...
                {paginatedMemories.map((memory, index) => (
                  <div
                    key={memory.id}
                    className={`border-b border-gray-50 hover:bg-purple-50 transition-all ${
                      memory.is_winner
                  ...
                  >
                    ...
                    <div className="hidden sm:grid grid-cols-12 gap-2 px-6 py-4 items-center text-sm">
                      ...
                      <div
                        className={`text-gray-600 text-xs ${hasExtraCol ? "col-span-2" : "col-span-2"}`}
                      >
                        <TextExpandable
                      ...
                      <div
                        className={`text-gray-500 text-xs ${hasExtraCol ? "col-span-1" : "col-span-2"}`}
                      >
                        <TextExpandable
                      ...
                      <div className="col-span-2 flex items-center justify-end gap-1">
                        ...
                        {activeTab === "approved" && (
                          <>
                            <button
                              onClick={() => handleSetWinner(memory.id)}
                              className="p-1.5 bg-yellow-400 text-white rounded-lg hover:bg-yellow-500"
                              title="Set as Winner"
                            >
                              <Trophy size={13} />
                            </button>
                            ...
                            <button
                              onClick={() => handleSetFunniest(memory.id)}
                              className="p-1.5 bg-orange-400 text-white rounded-lg hover:bg-orange-500"
                              title="Set as Funniest"
                            >
                              <Smile size={13} />
                            </button>
                            <button
                              onClick={() => handleSetHeartwarming(memory.id)}
                              className="p-1.5 bg-pink-400 text-white rounded-lg hover:bg-pink-500"
                              title="Set as Heartwarming"
                            >
