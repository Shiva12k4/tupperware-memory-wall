...
import { X, ChevronLeft, ChevronRight, Heart, Share2, Trophy, ChevronDown } from "lucide-react";
...
const MemorySlider = ({ memories, title, icon }) => {
  ...
  const next = () => setCurrentIndex((i) => Math.min(memories.length - 1, i + 1));
  ...
  if (memories.length === 0) return (
    <div className="mb-6">
      ...
      <div className="bg-white rounded-2xl p-8 text-center border border-purple-100">
        <p className="text-gray-400 text-sm">No memories found for this period</p>
      </div>
    </div>
  );

  const rankColors = ["bg-yellow-400", "bg-gray-400", "bg-orange-400", "bg-purple-400"];
  ...
  return (
    <div className="mb-8">
      ...
      <div className="hidden lg:flex items-center gap-3">
        ...
        <div className="flex-1 grid grid-cols-5 gap-3">
          {desktopVisible.map((memory, index) => (
            <div key={memory.id} className="relative">
              <div className={`absolute -top-2 -left-2 z-10 w-7 h-7 rounded-full flex items-center justify-center text-xs font-black text-white shadow-md ${
        ...
        <button
          onClick={() => setCurrentIndex((i) => Math.min(memories.length - 1, i + 5))}
...
const FeaturedModal = ({ onClose }) => {
  ...
  const periodLabels = { week: "This Week", month: "This Month", custom: "Custom" };
      ...
      >
        ...
        <div className="bg-gradient-to-r from-purple-700 to-pink-500 px-6 py-4 flex items-center justify-between flex-shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-lg font-black text-white">Featured Memories</h2>
              <p className="text-pink-200 text-xs">Top memories by likes & shares</p>
            </div>
          </div>
                  ...
                  {["week", "month", "custom"].map((p) => (
                    <button
                      ...
                      className={`w-full text-left px-4 py-2 text-sm transition-all hover:bg-purple-50 ${
                        period === p ? "text-purple-600 font-bold bg-purple-50" : "text-gray-600"
                      }`}
                    >
                      {p === "week" ? "This Week" : p === "month" ? "This Month" : "Custom Date"}
                    </button>
                  ))}
                </div>
              )}
